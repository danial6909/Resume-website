...
export default function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // آستانه اسکرول (مثلاً بعد از 200 پیکسل هدر ثابت ظاهر شود)
      const threshold = 200;
      if (window.scrollY > threshold) {
        setIsSticky(true);
      } else {
  ...
  // منطق شفافیت: فقط در صفحه اصلی و زمانی که هنوز استیکی نشده
  const getHeaderStyles = () => {
    // ۱. اگر حالت Sticky فعال باشد (در تمام صفحات)
    if (isSticky) {
      return "fixed top-0 bg-background/80 backdrop-blur-lg shadow-md border-border animate-slide-down";
    }
    
    // ۲. اگر در صفحه اصلی باشیم و بالای صفحه (شفاف)
    if (isHomePage) {
      return "absolute top-0 bg-transparent border-transparent";
    }

    // ۳. در صفحات دیگر وقتی بالای صفحه هستیم (رنگ ثابت تم)
    ...
    >
      <div className="container mx-auto px-4 h-full flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3 space-x-reverse">
          ...
          {user ? (
            <span className={`text-sm font-medium ${ (isHomePage && !isSticky) ? "text-white" : "text-foreground"}`}>
              {user.username}
            </span>
          ) : (
            <div className={(isHomePage && !isSticky) ? "dark" : ""}>
              <LoginBtn />
            </div>
          )}
        </div>
