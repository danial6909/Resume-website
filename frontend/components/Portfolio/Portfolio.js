...
// components/Portfolio/index.jsx
"use client";
import React, { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import TitleHeaderPages from "../TitleHeaderPages";
...
import { categories, projects } from "../../services/portfolioData"; // وارد کردن داده‌ها از فایل مجزا
...
const Portfolio = () => {
  ...
  return (
    <div className="min-h-screen bg-background py-10 px-6 mb-16 font-[vazir]" dir="rtl">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-8">
          <TitleHeaderPages title1="Portfolio" title2="همه پروژه‌ها" />
          
          {/* بخش فیلترها */}
          <div className="flex items-center gap-1 bg-surface p-1 rounded-full border border-border shadow-2xl">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setActiveCategory(cat)}
                className={`relative px-6 py-2.5 text-sm font-bold transition-colors duration-300 z-10 cursor-pointer ${
                  activeCategory === cat ? "text-background" : "text-text-faded hover:text-text-main"
                }`}
              >
                {activeCategory === cat && (
                  <motion.span
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, index) => (
              <ProjectCard 
                ...
                project={project} 
                index={index} 
                onViewDetails={() => setSelectedProject(project)} // پاس دادن تابع به کارت
              />
            ))}
          </AnimatePresence>
        </motion.div>
